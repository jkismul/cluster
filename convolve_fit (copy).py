...
kerns=[kerns0,kerns1]
kerns = 3*np.asarray(kerns)
#
#
# kerns=[]
# kernel_num_tsteps=402
# amps=[2,-2]
# t=np.linspace(0,40,1000)
#
# taus2 = [1.5, 1.5]
# taus = [1., 1.]
# kerns = [[np.zeros(402)],[np.zeros(402)]]
#
# # for i in range(1):
# for idx in range(2):
#     rv = st.norm.pdf(np.linspace(t_min, t_max, kernel_num_tsteps), loc=20,
#                                      scale=5)
#     amp_ = amps[idx]
#     normie = amp_ * rv
#     k_ = normie
...
#     # k_[int(kernel_num_tsteps / 2 ):] = amps[idx] * (np.exp(-t[:int(kernel_num_tsteps / 2)] / taus[idx]) - np.exp(
#     #     -t[:int(kernel_num_tsteps / 2)] / taus2[idx]))
#
#     kerns[idx] += k_
...
print(kernel_num_tsteps)
#####################################3
#FAKE SIGNAL
ttt=len(t)
ttt=kernel_num_tsteps
t=np.linspace(0,40,ttt)
data = np.zeros(ttt)
taus=[2.5,1.1]
amps=[1,-0.7]
krnl=[]
for idx in range(2):
    k_=np.zeros(ttt)
    k_[int(ttt/2):]=amps[idx]*np.exp(-t[:int(ttt/2)+1]/taus[idx])
    krnl.append((k_))
...
firing_rates = [amps[idx] * gauss(t2, t_locs[idx], stds[idx])
                #+ np.random.normal(0, 0.001, size=len(t))
                for idx in range(2)]

data = np.zeros(num_tsteps)
for idx in range(2):
    data += ss.convolve(firing_rates[idx], krnl[idx], mode="same")
args=[[np.zeros(len(data))],[data]]
#######################################
# </editor-fold>
        ...
        for idx in range(num_kernels_to_run):
            ...
            if input_args.fr == 'norm':
                ...
                for jj in range(num_norms):
                    firing_rate_ +=x0[idx * 3 + (jj * 3 + 2)]*st.skewnorm.pdf(np.linspace(t_min, t_max, num_tsteps),a,loc=x0[idx * 3 + jj * 3],scale=x0[idx * 3 + (jj * 3 + 1)])
...
ax_sig.plot(np.linspace(t_min, t_max, len(data)), data, c='k')
